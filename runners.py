...
from result_utils import build_result_context, render_metrics_table, select_worst_group_constrained
from sim_core import argmax_labelmap, colorize_composite, simulate_rods_and_unmix, to_uint8_gray
...
def _constraint_indices(labels, use_pool, fixed_fluorophores, allowed_fluorophores, fixed_probe_pairs):
    fixed_indices = []
...
def _render_selection_tables(use_pool, labels, sel_idx, worst_idx, predicted=False):
    title = "Selected fluorophores (with lasers, best)" if predicted else "Selected fluorophores (best)"
    st.subheader(title)
...
def _render_simulation_and_metrics(E_chan, colors, names):
    ...
    rmse_vals = [np.sqrt(np.mean((Ahat[:, :, r] - Atrue[:, :, r]) ** 2)) for r in range(len(names))]
...
def _run_emission_mode(
    *,
    ...
    fixed_indices, allowed_indices = _constraint_indices(
        labels, use_pool, fixed_fluorophores, allowed_fluorophores, fixed_probe_pairs
    )
    ...
    _render_selection_tables(use_pool, labels, sel_idx, worst_idx, predicted=False)

    selected_labels = [labels[j] for j in sel_idx]
    tops = _render_pairwise_table(E_norm[:, sel_idx], selected_labels, k_show)
    ...
    E_chan = cached_interpolate_E_on_channels(wl, E_norm[:, sel_idx], DETECTION_CHANNELS)
    names = [prettify_name(label) for label in selected_labels]
    rmse_vals, prop_vals, acc_vals = _render_simulation_and_metrics(E_chan, colors, names)
...
def _run_predicted_mode(
    *,
    ...
    fixed_indices0, allowed_indices0 = _constraint_indices(
        labels0, use_pool, fixed_fluorophores, allowed_fluorophores, fixed_probe_pairs
    )
    ...
    if laser_strategy == "Simultaneous":
        powers_A, _ = derive_powers_simultaneous(wl, dye_db, provisional_labels, laser_list)
    else:
        powers_A, _ = derive_powers_separate(wl, dye_db, provisional_labels, laser_list)
    ...
    if spec_res_mode == "33 detection channels (Valm lab)" and laser_strategy == "Simultaneous":
        ...
        E_norm_for_select = E_raw_all_33 / (np.linalg.norm(E_raw_all_33, axis=0, keepdims=True) + 1e-12)
    else:
        E_norm_for_select = E_norm_all

    fixed_indices_all, allowed_indices_all = _constraint_indices(
        labels_all, use_pool, fixed_fluorophores, allowed_fluorophores, fixed_probe_pairs
    )
    ...
    if spec_res_mode == "33 detection channels (Valm lab)" and laser_strategy == "Simultaneous":
        ...
        E_norm_sel = E_raw_sel / (np.linalg.norm(E_raw_sel, axis=0, keepdims=True) + 1e-12)
    ...
    # For the contrast table, sort worst labels by peak too.
    if worst_labels:
    ...
    if use_pool:
        ...
        html_two_row_table("Slot", "Fluorophore", [f"Slot {i + 1}" for i in range(len(fluors))], fluors)
    ...
    tops = _render_pairwise_table(E_norm_sel, labels_sel, k_show)
