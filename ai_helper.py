...
Rules:
- Do not invent fluorophores, probes, lasers, similarity metrics, or experimental results.
...
Warnings:
- Add a warning only when a requested probe/fluorophore/metric is not found or the request is truly unsupported.
...
- Base the explanation only on optimizer outputs, selected labels, top pairwise scores, similarity_metric, lasers, and metrics.
...
Rules:
- Maximum 4 bullet points.
- Base suggestions only on selected labels, top pairwise scores, similarity_metric, lasers, mode, and metrics.
- Do not assume the pairwise score is cosine similarity. Use the provided similarity_metric.
